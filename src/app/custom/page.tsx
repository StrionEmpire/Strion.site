...
// per-type defaults / visibility
const TYPE_PROFILE: Record<string, {
  showDimensions?: boolean;
  showTableSpecific?: boolean;
  showResinOptions?: boolean;
  showEdgeStyles?: boolean;
  showLighting?: boolean;
  showCabinetry?: boolean;
  showPanelGeometry?: boolean;
}> = {
...
const METALS = [
  // Gold & copper variants back in, with multiple ways:
...
// ------------------------------
// 3) Price model (simple but rational)
// ------------------------------
const BASE_BY_TYPE: Record<string, number> = {
  ...
  "Interior Trim / Moulding": 18, // per linear foot (we’ll estimate into subtotal)
  "Counters & Bar Tops": 2200, "Seating": 680, "Outdoor": 1200
};

const WOOD_MULTIPLIER: Record<string, number> = {
  // domestics
  "Pine": 0.8, "Ash": 1.0, "Red Oak": 1.0, "Maple": 1.05, "White Oak": 1.2,
  "Cherry": 1.15, "Walnut": 1.35,
  // exotics
...
const METAL_ADD: Record<string, number> = {
  "None": 0,
  "Gold (24K Leaf Accents)": 950,
  "Gold (18K Leaf Accents)": 650,
  "Gold (Flake/Veil Inlay)": 480,
  "Copper (Solid Bar Inlay)": 420,
  "Copper (Rod Edge Band)": 360,
  "Copper (Hammered Accents)": 280,
  "Brass (Polished)": 260,
  "Brass (Satin)": 240,
  "Bronze (Oil-Rubbed)": 250,
  "Stainless (Brushed)": 220,
  "Aluminum (Brushed)": 180,
};
...
function priceEstimate({
  itemType, width, length, thickness,
  wood, metal, geometry, resinPattern,
  includeLighting, cabinetryScopeLF
}: any) {
  let base = BASE_BY_TYPE[itemType] ?? 1200;

  // quick footage estimate for tables/panels (W x L in -> sqft)
  ...
  // trim by LF
  if (itemType === "Interior Trim / Moulding") {
    base = (BASE_BY_TYPE[itemType] ?? 18) * Math.max(20, cabinetryScopeLF || 20);
  }
  // wood & metal
...
// ------------------------------
// 4) SVG Preview (improved look)
// ------------------------------
function PreviewSVG({
  itemType, width, length, wood, metal, resinPattern, edgeStyle
}: any) {
  const W = 620, H = 360;
...
// ------------------------------
// 5) Field Components (simple)
// ------------------------------
function Label({children}:{children:any}) {
  return <div style={{fontSize:12,letterSpacing:0.4,opacity:0.8,marginBottom:6}}>{children}</div>;
}
...
export default function CustomConfigurator(){
  ...
  // keep query in sync if user jumps from categories often
      ...
      <section style={{display:"grid",gridTemplateColumns:"1.1fr 1fr",gap:24,marginTop:18}}>
        ...
        <div style={{border:"1px solid rgba(255,255,255,0.12)",borderRadius:14,padding:16,background:"linear-gradient(180deg, rgba(232,201,135,0.06), rgba(0,0,0,0.25))"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
            <div style={{gridColumn:"1 / -1"}}>
              ...
              <Select value={itemType} onChange={setItemType} options={BUILD_TYPES}/>
            </div>
            ...
            {profile.showDimensions && (
              <>
                ...
                <div>
                  ...
                  <Input value={thickness} onChange={setThickness} placeholder="e.g., 1.75"/>
                </div>
              </>
            )}
            ...
            <div style={{gridColumn:"1 / -1"}}>
              <Label>Metal Accents / Inlay</Label>
              <Select value={metal} onChange={setMetal} options={METALS}/>
            </div>

            {profile.showEdgeStyles && (
              <div>
            ...
            {profile.showResinOptions && (
              <div>
                <Label>Resin & Epoxy Pattern</Label>
            ...
            {(profile.showPanelGeometry || profile.showTableSpecific) && (
              <div style={{gridColumn:"1 / -1"}}>
                ...
                <Select value={geometry} onChange={setGeometry} options={GEOMETRIES}/>
              </div>
            )}
            ...
            {profile.showCabinetry && (
              <>
                <div>
                  <Label>Cabinet Front</Label>
                  <Select value={cabinetFront} onChange={setCabinetFront} options={CABINET_FRONTS}/>
                </div>
                <div>
                  <Label>Hardware</Label>
                  <Select value={hardware} onChange={setHardware} options={HARDWARE}/>
                </div>
                <div>
                  <Label>Estimated Linear Feet</Label>
                  <Input value={cabinetLF} onChange={setCabinetLF} placeholder="e.g., 24"/>
                </div>
              </>
            )}
          </div>
        ...
        <div style={{border:"1px solid rgba(255,255,255,0.12)",borderRadius:14,padding:12}}>
          ...
          <div style={{display:"flex",gap:10,marginTop:12,flexWrap:"wrap"}}>
            <a href="/work" className="px-4 py-2 rounded-lg" style={{border:"1px solid rgba(255,255,255,0.2)"}}>
              View Recent Builds
            </a>
            <a href="/categories" className="px-4 py-2 rounded-lg" style={{border:"1px solid rgba(255,255,255,0.2)"}}>
              Browse Categories
            </a>
          </div>
        </div>
